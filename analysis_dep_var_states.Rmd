...
3. Meaningful employment - in SW Minnesota, but not in the same EDR as their high school
4. Meaningful employment - in Minnesota, but not in Southwest Minnesota
...
We are going to assess each PersonID on which of those pathways they are at the time of their high school graduation, and each subsequent year up to 2019.
...
master.emp.states.yearly <- read_csv("Data/SLEDS/Masters/States/Master-emp-states-each-year.csv") %>%
  mutate(grad.year = fct_relevel(grad.year, "grad.year.0", "grad.year.1", "grad.year.2", "grad.year.3", "grad.year.4", "grad.year.5", "grad.year.6", "grad.year.7", "grad.year.8", "grad.year.9", "grad.year.10", "grad.year.11"),
         county.match = fct_relevel(county.match, "Meaningful emp - match", "Meaningful emp - no match", "MN emp record - not meaningful", "No MN emp record", "After 2019"),
         edr.match = fct_relevel(edr.match, "Meaningful emp - match", "Meaningful emp - no match", "MN emp record - not meaningful", "No MN emp record", "After 2019"),
         region.match = fct_relevel(region.match, "Meaningful emp - match", "Meaningful emp - no match", "MN emp record - not meaningful", "No MN emp record", "After 2019"),
         Dem_Desc.match = fct_relevel(Dem_Desc.match, "Meaningful emp - match", "Meaningful emp - no match", "MN emp record - not meaningful", "No MN emp record", "After 2019"),
         state.match = fct_relevel(state.match, "Meaningful emp - match", "Meaningful emp - no match", "MN emp record - not meaningful", "No MN emp record", "After 2019"))

head(master.emp.states.yearly)

names(master.emp.states.yearly)
```
...
grad.year.5 = six years after graduating high school
grad.year.10 = eleven years after graduating
