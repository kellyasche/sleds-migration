...
In this dataset, there were `r comma(cte.concentrator.cat %>% nrow(), accuracy = 1)` that were either a CTE participant or concentrator. This is below the ~37,000 that took at least one CTE course. This could mean that there 5,000 students that took a CTE course but did not graduate.
...
Of the total graduates from Southwest high schools, nearly half (46%) were a CTE concentrator or completor. This is a bit surprising. Lets see if there are any differences by rural-ness.
...
The cross-tabs show a significant difference between the percentage of students by categorization and RUCA categories. Our most rural counties have a significantly higher percentage of graduates that are CTE concentrators/completors (53%) than the other RUCA categories. The percentage decreases as the county group becomes more urban - the urban/town/rural mix county group had 39% of their graduates categorized as a CTE concentrator or completor. 

Our most rural counties also has a smaller percentage of graduates that took no CTE courses (23%) compared to the other RUCA groups. Urban/town/rural mix had the highest percentage with 32%. 
...
CrossTable(master.5$year, master.5$cte.achievement, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)
...
Okay, so it appears that 2006 is not a good year. This means I will have to get rid of 2006 in the entire data set. 
