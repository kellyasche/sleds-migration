...
With this dataset we can determine both the number of years each PersonID attended a school in Southwest Minnesota and their average CTE intensity during those years. But we will need to do them separately. 

Lets start with calculating the number of years each PersonID attended a Southwest MN high school. We will do this by counting the number of distinct PersonIDs when group by PersonID and year.

<br>

```{r number of years attended}
years.attended <- cte.intensity.enrollment %>%
...
In this dataset we have `r comma(years.attended %>% nrow(), accuracy = 1)` rows and `r comma(years.attended %>% ncol(), accuracy = 1)` columns.
