...
color.edr <- c("EDR 6E- Southwest Central" = "#e5f5f9", "EDR 6W- Upper Minnesota Valley" = "#bdc9e1", "EDR 8 - Southwest" = "#74a9cf")
...
head(nsc.enrollment.n.institutions)

names(nsc.enrollment.n.institutions)
```
...
head(nsc.enrollment.first.institution)

names(nsc.enrollment.first.institution)
```
...
head(nsc.enrollment.institution.types)

names(nsc.enrollment.institution.types)
```
...
nsc.enrollment.location.join <- nsc.enrollment.ipeds %>%
  ...
  left_join(counties.regions.1[,c(1,5,6,8)], by = c("countyfp", "statefp")) %>%
...
head(nsc.enrollment.location.join)

names(nsc.enrollment.location.join)
```
...
head(nsc.enrollment.same.ruca)

names(nsc.enrollment.same.ruca)
...
head(nsc.enrollment.same.edr)

names(nsc.enrollment.same.edr)
```
...
head(nsc.enrollment.same.pr)

names(nsc.enrollment.same.pr)
```
...
girafe(ggobj = attending.ps.pct.ruca.plot, width_svg = 10, height_svg = 6) %>%
...
```{r chart attending post secondary edr}
attending.ps.pct.edr <- master.9 %>%
  group_by(edr, attended.ps) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(edr) %>%
  mutate(pct = n / sum(n)) %>%
...
attending.ps.pct.edr.plot <- ggplot(attending.ps.pct.edr, aes(edr, pct, fill = attended.ps, group = attended.ps)) +
...
girafe(ggobj = attending.ps.pct.edr.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
```{r crosstabs attending post secondary edr}
CrossTable(master.9$edr, master.9$attended.ps, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)

```

<br>

# Summary of years between graduation and attending post-secondary{.tabset}
...
girafe(ggobj = years.between.grad.ps.dist.total.plot, width_svg = 10, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
```{r table years between hs and ps edr}
years.between.grad.ps.table.edr <- master.9 %>%
  filter(attended.ps == "Yes") %>%
  group_by(edr) %>%
  summarize(`Number of students` = n(),
            `Mean` = mean(attended.ps.years.hsgrad),
...
datatable(years.between.grad.ps.table.edr, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:6)))) %>%
  formatCurrency(2, "", digits = 0) %>%
  formatCurrency(3, "", digits = 2) %>%
  formatCurrency(7, "", digits = 4) 
...
```{r anova years between hs and ps edr}
years.between.grad.ps.anova.edr <- master.9 %>%
  filter(attended.ps == "Yes") %>%
  mutate(edr = str_sub(edr, 1, 6))

years.between.grad.ps.anova.edr.summary <- aov(attended.ps.years.hsgrad ~ edr, data = years.between.grad.ps.anova.edr)

summary(years.between.grad.ps.anova.edr.summary)

TukeyHSD(years.between.grad.ps.anova.edr.summary)
```
...
girafe(ggobj = n.colleges.dist.total.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
sectors <- read_xlsx("Data/SLEDS/NSC/IPEDS/institution-sectors.xlsx")
...
* Town/rural mix high school graduates attended public, 4-year institutions at a significantly lower rate than entirely rural and urban/town/rural mix students.
* The more urban a county group was, the high the percentage of the students attended a not-for-profit, 4 year institution.
* The more urban, the higher percentage of students attended a private, for-profit 4-year.
* Entirely rural and town/rural mix RUCA categories had student graduates attend public, 2-year at a significantly higher rate than urban/town/rural mix.
...
first.college.sector.ruca.plot <- ggplot(first.college.sector.ruca, aes(fct_rev(fct_reorder(Sector, pct)), pct, fill = Dem_Desc, group = Dem_Desc)) +
    ...
    labs(x="", y = "", color="", title = "Percent of students and first college attended by RUCA category")+
    scale_y_continuous(labels=scales::percent)+
    scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
    theme_bar+
...
girafe(ggobj = first.college.sector.ruca.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
* EDR 8 had a significantly higher percentage of students attend a public, 4-year institution.
* EDR 6E had a significantly higher percentage of students attend a not-for-profit, 4-year institution.
* EDR 6E had a significantly higher percentage of students attend a for-profit, 4-year institution.
* EDR 6W had a significantly higher percentage of students attend a public-2-year. 
...
first.college.sector.edr.plot <- ggplot(first.college.sector.edr, aes(fct_rev(fct_reorder(Sector, pct)), pct, fill = edr, group = edr)) +
    geom_col_interactive(position = "dodge", aes(data_id = data_id, tooltip = paste(edr, "\nInstitution sector: ", Sector, "\nNumber of graduates attending: ", comma(n, accuracy = 1), "\nPercent of students attending: ", percent(pct, accuracy = .1), sep = ""))) +
    geom_label(position = position_dodge(width = .9), aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5) +
    labs(x="", y = "", color="", title = "Percent of students and first college attended by edr category")+
    scale_y_continuous(labels=scales::percent)+
    scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
    theme_bar+
...
girafe(ggobj = first.college.sector.edr.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
girafe(ggobj = ps.same.ruca.ruca.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
# Summary of attending college in same EDR as high school
...
```{r attend same edr total}
ps.same.edr.total <- master.9 %>%
  filter(attended.ps == "Yes") %>%
  group_by(ps.in.same.edr) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(pct = n / sum(n),
         data_id = as.character(seq(n())))

ps.same.edr.total.plot <- ggplot(ps.same.edr.total, aes(fct_rev(fct_reorder(ps.in.same.edr, pct)), pct)) +
    geom_col_interactive(aes(data_id = data_id, tooltip = paste("College EDR location: ", ps.in.same.edr, "\nNumber of students: ", comma(n, accuracy = 1), "\nPercent of students: ", percent(pct, accuracy = .1), sep = ""))) +
    geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5) +
    labs(x="", y = "", color="", title = "Percent of students attending college within same EDR")+
    scale_y_continuous(labels=scales::percent)+
    scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
    theme_bar+
    theme(legend.position = "none",
...
girafe(ggobj = ps.same.edr.total.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
ps.same.edr.edr.plot <- ggplot(ps.same.edr.edr, aes(ps.in.same.edr, pct, group = edr, fill = edr)) +
    geom_col_interactive(position = "dodge", aes(data_id = data_id, tooltip = paste("edr category comparison between high school and college: ", "\nNumber of students: ", comma(n, accuracy = 1), "\nPercent of students: ", percent(pct, accuracy = .1), sep = ""))) +
    geom_label(position = position_dodge(width = .9), aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5) +
    labs(x="", y = "", color="", title = "Percent of students attending post-secondary of same edr\ncategory by edr category of high school")+
    scale_y_continuous(labels=scales::percent)+
    scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
    theme_bar+
    scale_fill_manual(values= color.edr,
...
girafe(ggobj = ps.same.edr.edr.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
ps.same.edr.ct.edr <- master.9 %>%
  filter(attended.ps == "Yes")

CrossTable(ps.same.edr.ct.edr$edr, ps.same.edr.ct.edr$ps.in.same.edr, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)
...
ps.same.pr.total.plot <- ggplot(ps.same.pr.total, aes(fct_rev(fct_reorder(ps.in.same.pr, pct)), pct)) +
    ...
    geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5) +
    labs(x="", y = "", color="", title = "Percent of students attending college within same planning region")+
    scale_y_continuous(labels=scales::percent)+
    scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
    theme_bar+
    theme(legend.position = "none",
...
girafe(ggobj = ps.same.pr.total.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
ps.in.MN.total.plot <- ggplot(ps.in.MN.total, aes(fct_rev(fct_reorder(ps.in.MN, pct)), pct)) +
    ...
    geom_label(aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5) +
    labs(x="", y = "", color="", title = "Percent of students attending college in Minnesota")+
    scale_y_continuous(labels=scales::percent)+
    scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
    theme_bar+
    theme(legend.position = "none",
...
girafe(ggobj = ps.in.MN.total.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
girafe(ggobj = ps.in.MN.ruca.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
...
girafe(ggobj = ps.in.MN.edr.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
