...
Not surprisingly, Agriculture, Food & Natural Resources was the careerfield with the highest percentage of courses taken with 34.5%. This was followed by business, management & administration with 24.9% and human services with 20.5% of CTE courses taken in that career field.
...
The county maps do show some differences in percentages. A significantly higher percentage of CTE courses taken were in Jackson (94.8%), Pipestone (63.6%) Cottonwood (55.2%), and Lincoln (53.7%) counties.

There were also higher percentages of courses taken in business, management and administration in Big Stone (53.7%), Murray (43.4%), and Renville (40.1%) counties.
...
cte.cf.concentration.year <- master.4 %>%
  select(PersonID, year, cte.careerfield.1, cte.careerfield.2, cte.careerfield.3, cte.careerfield.4, cte.careerfield.5, cte.careerfield.6, cte.careerfield.7, cte.careerfield.8, cte.careerfield.9, cte.careerfield.NULL) %>%
  gather(key = "cte.careerfield", value = "n.courses", 3:12) %>%
  group_by(year, cte.careerfield) %>%
  summarise(n = sum(n.courses)) %>%
  ungroup() %>%
  group_by(year) %>%
  mutate(pct = n / sum(n)) %>%
...
cte.cf.concentration.year.plot <- ggplot(cte.cf.concentration.year, aes(year, pct, color = cf.name)) +
  ...
  geom_label(data = filter(cte.cf.concentration.year, year == max(cte.cf.concentration.year$year)), aes(label = percent(pct, accuracy = .1)), show.legend = FALSE, size = 5) +
