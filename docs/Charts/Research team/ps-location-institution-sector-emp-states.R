...
ps.institutionsector <- ps.location.original %>%
  ...
  mutate(new.InstitutionSector = ifelse(ps.planning.region == "Southwest" & InstitutionSector %in% c(4,5,6,7,8,9), "Southwest, 2-year", InstitutionSector),
         ...
         new.InstitutionSetor = ifelse(ps.planning.region %in% c("Northwest", "Northeast", "Central", "Seven County Mpls-St Paul", "Southeast") & new.InstitutionSector %in% c(4,5,6,7,8,9), "MN, 2-year", new.InstitutionSector))

names(ps.institutionsector)
levels(ps.institutionsector$InstitutionSector)
