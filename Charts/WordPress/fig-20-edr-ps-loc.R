...
data <- master %>%
  select(edr, cte.achievement, ps.grad.InstitutionSector, ps.state, ps.pr) %>%
  ...
  mutate(prop = n / sum(n)) %>%
  ungroup() %>%
  filter(ps.loc %in% c("Southwest", "MN, outside SW", "Border state", "Far far away")) %>%
  mutate(ps.loc = fct_relevel(ps.loc, "No college", "Some college", "Southwest", "MN, outside SW", "Border state", "Far far away")) 
...
plot <- ggplot(data, aes(ps.loc, prop, group = edr, fill = edr)) +
  geom_col_interactive(position = "dodge", aes(data_id = n, tooltip = paste("5 years after high school", "\nEDR: ", edr, "\nPost-high school path: ", ps.loc, "\nNumber of individuals: ", comma(n), "\nProportion of individuals: ", percent(prop, accuracy = .1), sep = ""))) +
  geom_label(aes(label = percent(prop, accuracy = .1)), show.legend = FALSE, position = position_dodge(width = .9), color = "black", size = 4) +
  labs(x="", y = "", title = "Proportion of individuals by EDR and college graduation location", subtitle = "The proportions of individuals and their post-high school path vary significantly by EDR\n")+
  scale_y_continuous(labels=scales::percent,
                     limits = c(0, .35)) +
  scale_fill_manual(values = brewer.pal(n = 3, "Purples"),
...
withr::with_dir("Charts/WordPress", saveWidget(i.plot, file="fig-20-edr-ps-loc.html"))
