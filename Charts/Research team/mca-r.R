...
ggplot(data, aes(grad.year, MCA.R, fill = states)) +
  ...
  annotate(geom = "segment",
           ...
           yend = 3.1,
           arrow = arrow(length = unit(.25, "cm"))) +
  labs(x="", y = "", color="", title = "Boxplots of MCA - Reading scores by state", subtitle = "Meaningful employment in Southwest typically associated with lower MCA Reading scores")+
...
ggsave(filename = "Charts/Research team/mca-r.pdf", device = cairo_pdf, dpi = "print", width = 12, height = 6.5)
