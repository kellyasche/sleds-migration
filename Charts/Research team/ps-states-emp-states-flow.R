...
flow.5 <- data %>%
  ...
  make_long(ps.states, grad.year.5) %>%
  mutate(node = fct_relevel(node, "No MN emp record, not attending ps", "Not meaningful, not attending ps", "Attending ps", "Meaningful emp MN","Meaningful emp SW", "Far far away", "Border states", "MN - outside Southwest", "Southwest MN")) 

flow.10 <- data %>%
  filter(grad.year.10 != "After 2019") %>%
  filter(grad.year.10 != "Attending ps") %>%
  make_long(ps.states, grad.year.10) %>%
  mutate(node = fct_relevel(node, "No MN emp record, not attending ps", "Not meaningful, not attending ps", "Attending ps", "Meaningful emp MN","Meaningful emp SW", "Far far away", "Border states", "MN - outside Southwest", "Southwest MN")) 
...
ggplot(flow.10, aes(x = x,
  ...
  geom_sankey_label() +
  theme_bar +
  labs(x = "", y = "", title = "Flow of post-secondary location to states of\nemployment", subtitle = "Attending post-secondary outside of Southwest does not\nbenefit Southwest") +
...
ggsave(filename = "Charts/Research team/ps-states-emp-states-flow-10.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 6.5)
