...
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, grad.year %in% c("grad.year.1", "grad.year.5", "grad.year.10")), aes(cte.achievement, prop, fill = states)) +
  facet_wrap(~grad.year) +
  geom_bar(stat = "identity", position = position_stack()) +
  geom_label(data = filter(data, grad.year %in% c("grad.year.1", "grad.year.5", "grad.year.10")), aes(x = 1.5, y = .95, label = "Higher proportions"), show.legend = FALSE) +
  geom_label(data = filter(data, grad.year == "grad.year.10"), aes(x = 2, y = .15, label = "Significant growth"), show.legend = FALSE, fill = "#91bfdb") +
  labs(x="", y = "", title = "Proportion of CTE Achievement by employment state", subtitle = "CTE achievement is associated with higher levels of meaningful employment in Southwest") +
...
ggsave(filename = "Charts/Research team/cte-achievement.pdf", device = cairo_pdf, dpi = "print", width = 12, height = 6.5)
