...
data <- meaningful.emp.sw %>%
  ...
  mutate(grad.year = fct_relevel(grad.year, "grad.year.1", "grad.year.5", "grad.year.10"),
         highest.cred.level = fct_relevel(highest.cred.level, "Less than Associate Degree", "Associate degree", "Bachelor degree", "Master degree or higher")) %>%
  group_by(grad.year, highest.cred.level) %>%
  arrange(desc(states)) %>%
...
# Create chart ------------------------------------------------------------
ggplot(data = filter(data, grad.year %in% c("grad.year.5", "grad.year.10")), aes(highest.cred.level, prop, fill = states)) +
  facet_wrap(~grad.year) +
  geom_bar(stat = "identity", position = position_stack()) +
  geom_label(data = filter(data, grad.year == "grad.year.5"), aes(x = 1.5, y = .9, label = "Proportion decreases as\ncredential increases"), fill = "#d73027", show.legend = FALSE) +
  geom_segment(data = filter(data, grad.year == "grad.year.5"),
               x = 2.3, 
               xend = 4,
               y = .95,
               yend = .95,
               arrow = arrow(length = unit(.25, "cm"))) +
  geom_label(data = filter(data, grad.year == "grad.year.10"), aes(x = 1.5, y = .6, label = "Proportion increases as\ncredential increases"), fill = "#fc8d59", show.legend = FALSE) +
  geom_segment(data = filter(data, grad.year == "grad.year.10"),
               x = 2.3, 
               xend = 4,
               y = .6,
               yend = .6,
               arrow = arrow(length = unit(.25, "cm"))) +
  geom_label(data = filter(data, grad.year == "grad.year.10"), aes(x = 1, y = .15, label = "Major growth"), fill = "#91bfdb", show.legend = FALSE) +
  geom_segment(data = filter(data, grad.year == "grad.year.10"),
               x = 1.5, 
               xend = 4,
               y = .15,
               yend = .15,
               arrow = arrow(length = unit(.25, "cm"))) +
  labs(x="", y = "", title = "Proportion of highest credential level earned by state", subtitle = "Associate degree is major contributor to workforce in Southwest, while individuals with no MN employment record\ngrows") +
...
ggsave(filename = "Charts/Research team/highest-cred-level.pdf", device = cairo_pdf, dpi = "print", width = 12, height = 6.5)
