---
title: "National Student Clearinghouse - completion"
output:
  html_document:
    toc: true
    toc_float: true
...
Alright, now we are going to determine who graduated. Let's see what this data looks like.

<br>

```{r nsc completions original}
nsccompletions.original <- read_csv("Data/SLEDS/NSC/NSC_completions.csv") %>%
  rename(PersonID = 1) %>%
  select(1:7) %>%
...
head(nsccompletions.original)

names(nsccompletions.original)
```
...
head(year.first.last.graduated)

names(year.first.last.graduated)
```
...
head(grad.institution.sector)

names(grad.institution.sector)
```
...
```{r degree earned}
clean.degree <- read_xlsx("Data/SLEDS/NSC/NSC_Degree_Title_Cleaned.xlsx", sheet = 1)

highest.credential.code <- nsccompletions.original %>%
  left_join(clean.degree, by = c("DegreeTitle" = "CREDENTIAL_TITLE_REPORTED_TO_NSC")) %>%
  rename(credential.code = 9) %>%
  mutate(credential.code = as.factor(credential.code)) %>%
  select(-CREDENTIAL_LEVEL_DESC_CLEANED) %>%
  mutate(cred.level = ifelse(credential.code %in% c("CR", "PD", "UC"), "Less than Associate Degree",
         ...
         cred.level.code = ifelse(is.na(cred.level.code), 0, 1)) %>%
  group_by(PersonID) %>%
...
head(highest.credential.code)

names(highest.credential.code)
```
...
head(nsccompletions.location.join)

names(nsccompletions.location.join)
```
...
head(nsccompletions.same.ruca)

names(nsccompletions.same.ruca)
...
head(nsccompletions.same.edr)

names(nsccompletions.same.edr)
```
...
head(nsccompletions.same.pr)

names(nsccompletions.same.pr)
```
...
master.10 <- read_csv("Data/SLEDS/Masters/Master-9.csv") %>%
  ...
  rename(ps.grad = Graduated,
         ps.attend.InstitutionSector = InstitutionSector.x,
         ps.grad.InstitutionSector = InstitutionSector.y,
...
The crosstabs below indicate a relationship between the percentage of students that graduate from a post-secondary institution and the RUCA category of their high school.

In particular, individuals graduating from an entirely rural high school were significantly more likely (56%) to graduate from a post secondary institution compared to students who graduate from a town/rural mix and urban/town/rural mix high school (46% and 48% respectively).
...
Next, lets check to see if there are differences by EDR.

The crosstabs below indicate a relationship between the percentage of students that graduate from a post-secondary institution and the EDR of their high school.

In particular, individuals graduating from a high school in EDR 6E were significantly less likely (46%) to graduate from a post secondary institution compared to students who graduate from EDR 6W and EDR 8 (49% and 50% respectively).
...
The primary trend is that the average number of years between high school graduation and earning first credential increases as a high school becomes more urban - 3.23 years for entirely rural students and 3.62 for urban/town/rural mix students.
...
The primary trend is that the average number of years between high school graduation and earning first credential is sisgnificantly higher in EDR 6E (3.54 years) compared to EDR 6W (3.29 years) and EDR 8 (3.35 years).
...
First, let's look at it from a total dataset. Of the over 18,000 individuals that attended post-secondary in the dataset, nearly 40% went to a public, 4-year college, 35% went to a 2-year public college, 13% went to a private 4-year, and 9% graduated from a mixture of institution types. 
...
1. A higher percentage of students from an urban/town/rural mix high school completed college at a public, 4-year institution (45%) vs. individuals from entirely rural high schools (40%).
2. A higher percentage of students from entirely rural high schools completed college at a public, 2-year institution (40%) vs. individuals from urban/town/rural mix high schools (28%).
3. A higher percentage of students from urban/town/rural mix high schools completed college at a private, 4-year institution (16%) compared to individuals from entirely rural high schools (10%).
The primary trend is that the average number of years between high school graduation and earning first credential increases as a high school becomes more urban - 3.23 years for entirely rural students and 3.62 for urban/town/rural mix students.
...
Next lets see if there's a relationship between institution type and EDRs.

The Crosstabs indicates a relationship between the location of an individuals high school and the institution type from which they completed a degree.

Similary to the RUCA categories, the more urban an EDR is, the higher the percentage of individuals that completed a degree in a public, or not-for-profit, 4-year insititution compared to more rural EDRs. In addition, the more rural, the higher liklihood that an individual completed a degree at a public, 2-year institution.
