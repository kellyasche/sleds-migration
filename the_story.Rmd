...
plot <- ggplot(data = data, aes(new.ps.grad.InstitutionSector, prop, fill = states)) +
  ...
  geom_bar(stat = "identity", position = position_stack()) +
  geom_label(aes(label = percent(prop, accuracy = .1)), show.legend = FALSE, size = 5, color = "white") +
  labs(x="", y = "", title = "Proportion of individuals with no MN employment recordm in Minnesota\n by the institution sector from which they graduated", subtitle = "A significantly larger proportion of individuals with no college experience do not\nhave a Minnesota employment record\n") +
  scale_y_continuous(labels=scales::percent) +
  scale_fill_manual(values = states.color,
                    guide = guide_legend(ncol = 2)) +
  theme_bar+
...
# Estimates on no MN employment record

We know the potential scenarios;

1. They could be federally employed
2. They could be in the military and not employed
3. Could be self-employed
4. Not participating in the labor force
5. Passed away.

It seems like we could just use the percentage of people that are participating in the labor force at each age group and apply it to our dataset at five years after graduating high school.

The ages that we most likely have in our dataset are 18 - 29. So let's first get a distribution of approximate ages at five years after high school.

```{r age distribution five years after high school}
age <- read_csv("Data/SLEDS/Masters/Master.csv") %>%
  select(PersonID, grad.year) %>%
  left_join(original, by = "PersonID") %>%
  select(PersonID, grad.year, grad.year.5) %>%
  mutate(age = (2019 - grad.year) + 18)

age.5 <- age %>%
  filter(grad.year.5 == "No MN emp record, not attending ps")
```

At five years after high school, the total number of individuals that did not have a Minnesota employment record was `r comma(nrow(age.5))`. Now, could we just use the average percentage of students that have meaningful workforce participation in Southwest and Minnesota? Sure, let's do it.

```{r avg prop meaningful workforce in SW and MN}
meaningful.wf.sw.mn <- master %>%
  select(grad.year.5) %>%
  filter(grad.year.5 != "After 2019") %>%
  ...
  ungroup() %>%
  mutate(pct = n / sum(n)) %>%
  filter(grad.year.5 %in% c("Meaningful emp SW", "Meaningful emp MN")) %>%
  mutate(avg = mean(pct))


```

The average percentage of individuals that had meaningful workforce participation in SW and MN is `r percent(meaningful.wf.sw.mn$avg, accuracy = .1)`. 
