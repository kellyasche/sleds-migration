...
graduates <- enrollment.original %>%
  ...
  mutate(year = str_sub(K12EnrTimeID, 1, 4),
         year = as.integer(year)) %>%
  select(PersonID, K12OrganizationID, year, Gender, LimitedEnglishProficiencyIndicator, HomelessIndicator) %>%
...
k12.enr.pct.total <- k12.enrollment.master %>%
  ...
  gather(key = "indicator", value = "value", 4:10) %>%
  group_by(indicator) %>%
...
As expected, nearly half (49.3%) of the observations are female. Over a third (37.4%) of the observations have been eligible for free or reduced lunch at some point in the dataset. Also, nearly a third (29.5%) of observations participated in a PSEO course. 8.2% of observations indicated that there was a non-english speaker at home, 2% had limited english proficiency and 0.4% indicated that were homeless at some point.
